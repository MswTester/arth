...
async function bootstrap() {
  ...
  if (!existsSync(config.home)) {
    console.log(
      c('yellow', '[Warning]'),
      'Home directory not found. Creating one...',
    );
  ...
  console.log(
    c('sky', `========== ARTH ${process.env.npm_package_version} ==========`),
  );
  console.log(
    c('sky', 'Server running on port'),
    c('yellow', argv.port.toString()),
    c('sky', 'with PIN'),
    c('yellow', argv.pin),
  );
  ...
  if (!isAndroid())
    console.warn(
      c('yellow', '[Warning]'),
      'This OS is not Android, so some features might not be available.',
    );
}
bootstrap();
