...
type CloudMessage = { message?: string; error?: string };
...
export class CloudController {
  ...
  async write(
    ...
    if (path === '/')
      return Promise.reject({ error: 'Cannot write to root directory' });
    try {
  ...
  @Get('delete')
  async delete(@Query('path') path: string): Promise<CloudMessage> {
    if (path === '/')
      return Promise.reject({ error: 'Cannot delete root directory' });
    try {
  ...
  @Post('deleteMany')
  async deleteMany(@Body('paths') paths: string[]): Promise<CloudMessage> {
    if (paths.includes('/'))
      return Promise.reject({ error: 'Cannot delete root directory' });
    try {
  ...
  async move(
    ...
    if (path === '/')
      return Promise.reject({ error: 'Cannot move root directory' });
    try {
  ...
  async moveMany(
    ...
    try {
      ...
      paths.forEach((path) =>
        this.cloudGateway.fileCreated(
          splitPath(join(to, splitPath(path).pop())),
        ),
      );
  ...
  async copy(
    ...
    if (path === '/')
      return Promise.reject({ error: 'Cannot copy root directory' });
    try {
  ...
  async rename(
    ...
    if (path === '/')
      return Promise.reject({ error: 'Cannot rename root directory' });
    try {
